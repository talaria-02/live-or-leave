...
접속 대상 IP·계정명은 `cd.yml`에 하드코딩하지 않고 `GCE_HOST`/`GCE_USER` GitHub
Secret으로 관리한다. VM을 새로 만들어 고정 IP가 바뀌면, 코드 수정 없이
`gh secret set GCE_HOST` / `gh secret set GCE_USER`로 Secret 값만 갱신하면 된다.
