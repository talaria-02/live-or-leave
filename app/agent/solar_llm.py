...
- extra_categories: 사용자가 실제 시설/업종명을 명시적으로 말한 경우(예: "버거집",
  ...
  __CATEGORIES__
- required_filters: '필수 요구사항' 구역에서 언급된 조건을 아래 3개 타입 중 하나로
...
class SolarLLM:
    ...
    def parse_intent(self, text: str) -> ParsedIntent:
        ...
        try:
            ...
            # 필수/선택 구역을 나눠 각 검증을 그 구역에만 적용한다 — 안 나누면
            # "선택 요구사항"에만 적힌 단어(예: "공원")가 "텍스트 어딘가에 있긴
            # 하다"는 이유로 required_filters 존재-검증을 통과해버려, 하드
            # 필터가 아닌 걸 하드 필터로 취급하는 버그가 난다(실제로 "헬스장
            # 있어야 함" + "쉬는 날엔 공원에서 걷고 싶어요"를 넣으면 공원까지
            # 필수 취급돼 대부분의 동이 실격되는 형태로 나타났다). 마커가 없는
            # 자유 문장은 split_required_optional이 ("", text)를 반환해 이전
            # 동작(전체 텍스트 기준)과 100% 동일하게 유지된다.
            ...
            extra = explicitly_requested_categories(optional_part, extra)
            ...
            # required_part만 본다(마커가 없으면 빈 문자열이라 전체 text로
            # 폴백 — split_required_optional의 "마커 없으면 기존 동작 유지"
            # 규약과 맞춘다). optional_part/전체 text를 그냥 쓰면 위에서
            # 설명한 오염이 재발한다.
            verified_categories = explicitly_requested_categories(
                required_part or text, [c.category for c in clauses if c.type == "category"])
            clauses = [c for c in clauses
                       if c.type != "category" or c.category in verified_categories]

            needs_clarification = bool(data.get("needs_clarification", False))
            clarify_question = data.get("clarify_question")
            # gu/near/category 중 하나라도 구체적 필터가 이미 파싱됐으면
            # 애매함 휴리스틱으로 되묻기를 강제하지 않는다 — should_force_clarification은
            # 4개 카테고리·업종 언급만 볼 뿐 gu/near는 모르기 때문이다.
            ...
            return ParsedIntent(
                ...
                require_large_hospital=(
                    ...
                    and explicitly_requires_large_hospital(required_part or text)
                ),
