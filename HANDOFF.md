...
1. **Docker 멀티스테이지 빌드로 컨테이너화** → **GCE VM에 docker compose로 배포**
   (외부에서 접속 가능해야 함). `main.py`(FastAPI+SSE)와 `streamlit_app.py`
   (지도 UI) 둘 다 완성된 상태라, 결과 화면(FE)도 배포본에 포함해 배포된
   서비스에서 바로 입력·결과 확인이 가능해야 함.
2. **GitHub Actions로 CI(테스트 자동 실행) + CD(GCE 자동 배포) 구성.**
   API 키(`UPSTAGE_API_KEY` 등)는 GitHub Secrets/환경변수로 분리, 코드에 하드코딩 금지.
3. **핵심 시나리오 30개 구성** — `nvidia/Nemotron-Personas-Korea` 기반 현실적인
   질문으로 만들어 실제 Solar API 대상 검증.
4. **(선택, 가점) LLMOps 운영 안정성 개선 1종 이상 적용.**
5. **(여유 있으면)** 반경 1km 적절성 검증(큰 행정동에서 부족할 수 있음),
