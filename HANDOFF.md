...
- **Day4 배포 준비 1차 완료**: `Dockerfile`, `docker-compose.yml`, `.dockerignore`,
  `.env.example`, `.github/workflows/ci.yml`, `docs/deploy-gce.md`를 추가했다.
  FastAPI API(8000)와 Streamlit UI(8501)를 같은 Docker 이미지에서 실행하고,
  GitHub Actions는 테스트와 Docker 이미지 빌드를 자동 실행한다. 로컬에서
  `docker build -t live-or-leave:local-test .` 성공, 임시 컨테이너의
  `GET /health`가 `{"status":"ok"}`로 응답하는 것까지 확인했다.
...
1. **Docker 멀티스테이지 빌드 + docker compose 1차 구성 완료.**
